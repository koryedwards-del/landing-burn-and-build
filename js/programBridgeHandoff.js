...
const DRAFT_PROGRAM_KEY = 'bnb_program_draft';
...
export function loadProgramBridge() {
  try {
    const raw = sessionStorage.getItem(DRAFT_PROGRAM_KEY) || sessionStorage.getItem(BUILT_PROGRAM_KEY);
    if (!raw) return null;
    return JSON.parse(raw);
  } catch {
    sessionStorage.removeItem(DRAFT_PROGRAM_KEY);
